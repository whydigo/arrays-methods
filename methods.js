...
function getUserById(array, id) {
    const findUser = array.find(function (item) {
        if (item.id === id) {
...
function getGeoKey(array, id) {
    const findGeo = array.find(function (item) {
        if (item.id === id) {
...
function removeAddresses(array) {
    const minusAdress = array.filter(function (item) {
...
function deleteUser(array, id) {
    const minusUser = array.filter(function (item) {
        if (item.id === id) {
            delete item
        } else {
            return item
...
function getUsersCompany(array, id) {
        const findComp = array.find(function (item) {
            if (item.id === id) {
                console.log(item.company)
            }
        })
...
console.log(getUsersCompany(users, 5))
...
function changePhone(array, id, phone) {
    const newPhoneNumber = array.map(function(item) {
        if (item.id === id) {
            item.phone = phone
            return item
        }
...
console.log(changePhone(users, 2, 78787))